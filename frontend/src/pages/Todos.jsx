...
const Todos = () => {
  ...
  const protectTodoPage = async () => {
    ...
    if (!token) {
      ...
      try {
        const response = await fetch(`https://mern-todo-app-beta.vercel.app/pages/todos`, {
          method: "GET",
          headers: {
            "Authorization": token, 
          }
  ...
  const deleteRequest = async (id) => {
    const response = await fetch(`hhttps://mern-todo-app-beta.vercel.app/pages/todos/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
